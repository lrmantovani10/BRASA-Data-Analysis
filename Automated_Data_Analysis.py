...
path = 'Data//'
fb = pd.read_csv(path+'Facebook.csv')
fb1 = pd.read_csv(path+'Posts.csv')
lk1 = pd.read_csv(path+'L_Followers.csv')
lk2 = pd.read_csv(path+'L_Updates.csv')
lk3 = pd.read_csv(path+'L_Visits.csv')
m_choice = ''
r = list()
l_names = list()
f_path = path+'T_Teams.csv'
...
def run(f_index):
    ...
    if f_index==0:
        ...
        try:
            n_choice = int(input('Digite um número: '))
        except:
